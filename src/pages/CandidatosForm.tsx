...
import { EstadosCandidato, Candidato, FuentesCandidato } from '../types'
...
export default function CandidatosForm() {
  ...
  function set<K extends keyof Candidato>(k:K, v:any){ setForm(prev=>({...prev, [k]: v })) }
  ...
  async function submit(e:any){
    ...
    try {
      ...
      alert('Guardado ✅')
      setForm(f=>({...f, CandidatoID: genId(), Notas:''}))
    ...
    <div className="max-w-5xl mx-auto p-4">
      <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-2xl shadow-soft">
        ...
        <Select label="Fuente" value={form.Fuente} onChange={e=>set('Fuente', e.target.value)}>
          {FuentesCandidato.map(s=><option key={s}>{s}</option>)}
        </Select>
